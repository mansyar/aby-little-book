// Publishes versioned 3D story packages into the static output.
// Reads art/manifest/*.json and copies each scene GLB (and any texture
// payload) to dist/stories/<packageId>/<src>, so the URLs the app prepares
// (/stories/<packageId>/glb/*.glb), the service worker serves offline, and
// the reader loads all resolve against the deployed static files. Runs as
// `postbuild`: `pnpm build` = `vite build` followed by this publisher.
...
function publish(manifestPath) {
  ...
  const packageDir = join(DIST, 'stories', manifest.packageId);
  let bytes = 0;
